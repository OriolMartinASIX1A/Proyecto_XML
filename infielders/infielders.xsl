...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/top_infielders">
    <html>
      <head>
        <style>
          body {
        ...
        <table>
          ...
          <xsl:for-each select="jugador">
            <tr>
              ...
              <!-- Doble play sólo para infielders, si es catcher queda vacío -->
              <td>
                <xsl:if test="defensa[@tipo='infielder']">
                  <xsl:value-of select="defensa/double_play_pct"/>
                </xsl:if>
              </td>
              <!-- Catchers: CS %, Pop Time, Framing -->
              <td>
                <xsl:if test="defensa[@tipo='catcher']">
                  <xsl:value-of select="defensa/caught_stealing_pct"/> / 
                  <xsl:value-of select="defensa/pop_time"/> / 
                  <xsl:value-of select="defensa/framing_runs"/>
                </xsl:if>
              </td>
            </tr>
          </xsl:for-each>
